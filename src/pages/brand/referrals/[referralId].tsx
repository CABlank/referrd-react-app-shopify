...
const ReferralDetails: React.FC = () => {
  ...
  const totalShares = shares.length; // Total users who signed up
  const totalConversions = conversions.length; // Total users who converted

  const metrics = {
    clicks: customer?.click_count ?? 0, // Total signups (shares)
      ...
      <div className="flex flex-col justify-center items-center mx-auto gap-4 sm:p-4">
        ...
        <div className="flex flex-col w-full overflow-hidden rounded-2xl bg-[#f3f3f3]">
          <div className="flex w-full gap-4 flex-col lg:flex-row">
            <DetailSection
              customer={customer}
              referralCode={null} // Assuming no referral code data
              campaign={campaign}
            />
