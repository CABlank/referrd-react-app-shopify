...
var MobileCustomizationPanel = function (_a) {
    ...
    var renderFormFields = function (element) {
        return fieldConfigs[element.type].map(function (field) { return (<FormField key={field.name} label={field.label} name={field.name} type={field.type} value={getFieldValue(element, field.name)} onChange={function (e) { return handleChange(e, element); }} onBatchChange={field.type === "margin" || field.type === "padding"
            ...
            } options={field.options} style={element.type === "image"
                ? { paddingTop: "4px", paddingBottom: "4px" }
                : undefined}/>); });
    };
    return (<div className="p-4 bg-white rounded-lg mb-4">
      <h2 className="font-semibold">Customize Elements</h2>
      {elements.map(function (element) { return (<div key={element === null || element === void 0 ? void 0 : element.id} className="mt-4">
          <div className="flex justify-between items-center cursor-pointer py-2 bg-white" onClick={function () { return element && toggleElement(element.id); }}>
          ...
          {openElementId === (element === null || element === void 0 ? void 0 : element.id) && (<div className="space-y-4">
              {(element === null || element === void 0 ? void 0 : element.type) === "image" ? (<div className="image-options">
                  <label className="block text-sm font-medium text-black/50 mt-4 my-2">
                    Upload Image
                  </label>
                  <ImageUpload imageUrl={element.imageUrl || ""} onImageChange={function (file) {
                        return element && handleImageChange(file, element);
                    }} onRemoveImage={function () { return element && handleRemoveImage(element); }}/>
                  {element && renderFormFields(element)}
                </div>) : (element && renderFormFields(element))}
            </div>)}
        </div>); })}
    </div>);
};
