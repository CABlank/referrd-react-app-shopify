...
export var useDropWrapper = function (enableDragAndDrop, step, currentElements, setCurrentElements, setHoverIndex, hoverIndex) {
    ...
    var _a = useDrop({
        ...
        hover: function (item, monitor) {
            ...
            var hoverClientX = clientOffset
                ...
                : 0;
            var newHoverIndex = Math.floor((hoverClientX / hoverBoundingRect.width) * currentElements.length);
            newHoverIndex = Math.max(0, Math.min(currentElements.length, newHoverIndex));
            setHoverIndex(newHoverIndex);
        },
        drop: function (item) {
            if (!enableDragAndDrop)
                return;
            var elements = __spreadArray([], currentElements, true);
            var setElements = setCurrentElements;
    ...
    return { isOver: isOver, drop: enableDragAndDrop ? drop : null, containerRef: containerRef };
};
