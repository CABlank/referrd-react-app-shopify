...
var StepTwo = function (_a) {
    var elements = _a.elements, setElements = _a.setElements, moveElement = _a.moveElement, hoverIndex = _a.hoverIndex, elementWidth = _a.elementWidth, onRemove = _a.onRemove, view = _a.view, _b = _a.url, url = _b === void 0 ? "" : _b, onClose = _a.onClose;
    ...
    var renderIcon = function (IconComponent, bgColor, boxShadow, index) {
        var iconSize = view === "mobile" ? "w-6 h-6" : "w-8 h-8";
        return (<div key={index} // Add unique key prop here
         className={"cursor-pointer flex justify-center items-center flex-grow-0 flex-shrink-0 relative gap-1 rounded-[5.33px] ".concat(bgColor, " p-1 ").concat(iconSize)} style={{ boxShadow: boxShadow }}>
        <IconComponent />
      </div>);
    };
    ...
    return (<>
      {view === "desktop" && (<div className="flex justify-center items-center flex-grow-0 flex-shrink-0 gap-4 relative">
          ...
          <div className="flex flex-col justify-center items-center w-[720px]">
            ...
            <div className="flex flex-col justify-center items-center gap-0">
              <div className="flex justify-start items-start gap-10">
                ...
                return renderIcon(icon.component, icon.color, icon.shadow, index);
            })}
                </div>
                <div className="flex justify-start items-start gap-10">
                  {socialMediaIcons
          ...
          <div className="flex justify-between">
            {elements.map(function (element, index) { return (<ElementWrapper key={element.id} // Ensure each element has a unique key
             index={index} element={element} moveElement={moveElement} elementWidth={elementWidth} hoverIndex={hoverIndex} onRemove={onRemove} showRemoveButton={false}/>); })}
      ...
      {view === "mobile" && (<div className="flex flex-col justify-center items-center pt-2 relative">
          ...
          <div className="flex flex-wrap justify-between w-full mt-2">
            {elements.map(function (element, index) { return (<ElementWrapper key={element.id} // Ensure each element has a unique key
             index={index} element={element} moveElement={moveElement} elementWidth={elementWidth} hoverIndex={hoverIndex} onRemove={onRemove} showRemoveButton={false}/>); })}
            {hoverIndex === elements.length && (<div key={"hoverIndex-".concat(elements.length)} // Add a unique key
             className="flex-shrink-0" style={{ width: elementWidth, borderRight: "2px solid red" }}/>)}
          </div>
        </div>)}
    </>);
};
