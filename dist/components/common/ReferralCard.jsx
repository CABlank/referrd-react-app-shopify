...
var ReferralCard = function (_a) {
    var data = _a.data;
    return (<div className="flex flex-col overflow-hidden gap-4 rounded-2xl bg-white w-full">
      ...
      {data.map(function (item, index) { return (<React.Fragment key={index}>
          <ReferralItem {...item}/>
          {index < data.length - 1 && (<hr className="w-full border-t border-black/15"/>)}
        </React.Fragment>); })}
    </div>);
};
var ReferralItem = function (_a) {
    var name = _a.name, test = _a.test, code = _a.code, date = _a.date;
