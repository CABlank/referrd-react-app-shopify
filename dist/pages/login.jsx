...
var Login = function () {
    var _a = useState(true), isLoginActive = _a[0], setIsLoginActive = _a[1]; // State to toggle between login and sign up
    var router = useRouter();
    var _b = useSession(), login = _b.login, session = _b.session, loading = _b.loading;
    ...
    return (<AuthLayout title="Referral Rewards Await!" subtitle="Discover how businesses are monetizing word of mouth">
      <div className="flex flex-col justify-start items-center flex-grow-0 flex-shrink-0 w-full sm:w-[468px] overflow-hidden gap-8 pb-8 rounded-2xl bg-white">
        ...
        <div className="flex justify-between items-center self-stretch flex-grow-0 flex-shrink-0">
          <div className={"flex justify-center cursor-pointer items-center self-stretch flex-grow relative px-2.5 py-5 border-t-0 border-r-0 ".concat(isLoginActive
            ? "border-b-[1.5px] border-[#10ad1b]/50"
            : "border-b-[0.8px] border-black/30")} onClick={function () { return setIsLoginActive(true); }} style={{ transition: "background-color 200ms" }} onMouseEnter={function (e) {
            return (e.currentTarget.style.backgroundColor =
                "rgba(71, 183, 117, 0.2)");
        }} onMouseLeave={function (e) { return (e.currentTarget.style.backgroundColor = ""); }}>
            <p className={"flex-grow-0 flex-shrink-0 text-xl font-medium text-left ".concat(isLoginActive ? "text-[#10ad1b]" : "text-black/30")}>
              Log In
            </p>
          </div>
          <div className={"flex justify-center cursor-pointer items-center self-stretch flex-grow relative px-2.5 py-5 border-t-0 border-r-0 ".concat(!isLoginActive
            ? "border-b-[1.5px] border-[#10ad1b]/50"
            : "border-b-[0.8px] border-black/30")} onClick={function () { return setIsLoginActive(false); }} style={{ transition: "background-color 200ms" }} onMouseEnter={function (e) {
            return (e.currentTarget.style.backgroundColor =
                "rgba(71, 183, 117, 0.2)");
        }} onMouseLeave={function (e) { return (e.currentTarget.style.backgroundColor = ""); }}>
            <p className={"flex-grow-0 flex-shrink-0 text-xl font-medium text-left ".concat(!isLoginActive ? "text-[#10ad1b]" : "text-black/30")}>
